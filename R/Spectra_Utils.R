...
PerformRTcorrection <- function(mSet, param){
  ...
  if (.on.public.web & !optimize_switch){
    ...
    print_mes <- paste("Retention time correction is running.");    
    write.table(print_mes,file="metaboanalyst_spec_proc.txt",append = T,row.names = F,col.names = F, quote = F, eol = "");
    write.table(60.0, file = paste0(fullUserPath, "log_progress.txt"),row.names = F,col.names = F);
  ...
  if (.on.public.web & !optimize_switch){
    print_mes <- c("Done !");    
    write.table(print_mes,file="metaboanalyst_spec_proc.txt",append = T,row.names = F,col.names = F, quote = F, eol = "\n");
    write.table(68.0, file = paste0(fullUserPath, "log_progress.txt"),row.names = F,col.names = F);
  } else {
    message("Done !")
  }
...
adjustRtime_peakGroup <- function(mSet, param, msLevel = 1L) {
  ...
  if (!is.null(mSet[["onDiskData"]])){
    ...
    Subset_QC <- which(mSet[[format]]@phenoData@data[["sample_group"]]=="QC");
  ...
  if (is.null(pkGrpMat) & !optimize_switch){
    ...
    write.table(print_mes,file="metaboanalyst_spec_proc.txt",append = T,row.names = F,col.names = F, quote = F, eol = "\n");
    write.table(65.0, file = paste0(fullUserPath, "log_progress.txt"),row.names = F,col.names = F);
  ...
  if (.on.public.web & !optimize_switch){
    ...
    print_mes <- c("Applying retention time adjustment to the identified chromatographic peaks ... ");    
    write.table(print_mes,file="metaboanalyst_spec_proc.txt",append = T,row.names = F,col.names = F, quote = F, eol = "");
...
mSet.obiwarp <- function(mSet, object, param0) { ## Do not use the params defined by user for now!
  ...
  Subset_QC <- which(mSet[["onDiskData"]]@phenoData@data[["sample_group"]]=="QC");
  # files_list <- mSet[["onDiskData"]]@processingData@files;
...
.getChromPeakData <- function(object, peakArea, sample_idx,mzCenterFun = "weighted.mean",cn = c("mz", "rt", "into", "maxo", "sample")) {
  ...
  if (.on.public.web){
    ...
    print_mes_tmp <- paste0("Requesting ", nrow(res), " peaks from ",basename(MSnbase::fileNames(object)), " ... ");    
    #write.table(print_mes,file="metaboanalyst_spec_proc.txt",append = T,row.names = F,col.names = F, quote = F, eol = "");
...
getAllPeakEICs <- function(mSet, index=NULL){
  ...
  if(nfiles == 1){
    ...
    if (.on.public.web){
      print_mes <- paste0('Generating EIC\'s.');    
      write.table(print_mes,file="metaboanalyst_spec_proc.txt",append = T,row.names = F,col.names = F, quote = F, eol = "");
    } else {
    ...
    for (f in 1:nfiles){
      ...
      if (.on.public.web){
        print_mes <- paste("Detecting ",basename(mSet$xcmsSet@filepaths)[f]," ... ");    
        write.table(print_mes,file="metaboanalyst_spec_proc.txt",append = T,row.names = F,col.names = F, quote = F, eol = "");
      } else {
      ...
      if(length(idx.peaks) == 0){
        if (.on.public.web){
          print_mes <- paste0("0 peaks found!");
          write.table(print_mes,file="metaboanalyst_spec_proc.txt",append = T,row.names = F,col.names = F, quote = F, eol = "\n");
        } else {
...
calcCiS<- function(mSet, EIC=EIC, corval=0.75, pval=0.05, psg_list=NULL ){
  ...
  if(npspectra < 1){
    ...
    if (.on.public.web){
      ...
      print_mes <- paste0('Calculating peak correlations in 1 group.');    
      write.table(print_mes,file="metaboanalyst_spec_proc.txt",append = T,row.names = F,col.names = F, quote = F, eol = "\n");
...
calcPC.hcs <- function(mSet, ajc=NULL,psg_list=NULL) {
  ...
  if(is.null(psg_list)){
    if (.on.public.web){
      ...
      print_mes <- paste('Calculating graph cross linking in', npspectra, 'Groups...');    
      write.table(print_mes,file="metaboanalyst_spec_proc.txt",append = T,row.names = F,col.names = F, quote = F, eol = "\n");
  ...
  if (.on.public.web){
    ...
    print_mes <- paste("New number of ps-groups: ",length(pspectra));    
    write.table(print_mes,file="metaboanalyst_spec_proc.txt",append = T,row.names = F,col.names = F, quote = F, eol = "\n");
...
findAdducts <- function(mSet, ppm=5, mzabs=0.015, multiplier=3, polarity=NULL, rules=NULL, 
  ...
  if(mSet$AnnotateObject$sample == 1 && length(rownames(mSet$xcmsSet@phenoData)) == 1){
    ...
    if (.on.public.web){
      ...
      print_mes <- paste0("Generating peak matrix for peak annotation!");    
      write.table(print_mes,file="metaboanalyst_spec_proc.txt",append = T,row.names = F,col.names = F, quote = F, eol = "\n");
  ...
  if(!(mSet$AnnotateObject$polarity == "")){
    ...
    if (.on.public.web){
      ...
      print_mes <- paste("Polarity is set in annotaParam:", mSet$AnnotateObject$polarity);    
      write.table(print_mes,file="metaboanalyst_spec_proc.txt",append = T,row.names = F,col.names = F, quote = F, eol = "\n");
    ...
    if(is.null(rules)){
      if(!is.null(mSet$AnnotateObject$ruleset)){
        ...
        if (.on.public.web){
          ...
          print_mes <- paste0("Ruleset could not read from object! Recalculating...");
          write.table(print_mes,file="metaboanalyst_spec_proc.txt",append = T,row.names = F,col.names = F, quote = F, eol = "\n");
  ...
  if (runParallel == 1) { ## ... we run in parallel mode
    if(is.null(psg_list)){
      ...
      if (.on.public.web){
        ...
        print_mes <- paste('Calculating possible adducts in',npspectra,'Groups... ');  
        write.table(print_mes,file="metaboanalyst_spec_proc.txt",append = T,row.names = F,col.names = F, quote = F, eol = "\n");
      ...
      if (.on.public.web){
        ...
        print_mes <- paste('Calculating possible adducts in',length(psg_list),'Groups... ');  
        write.table(print_mes,file="metaboanalyst_spec_proc.txt",append = T,row.names = F,col.names = F, quote = F, eol = "\n");
