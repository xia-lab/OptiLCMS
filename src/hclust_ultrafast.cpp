...
void generate_R_dendrogram(int * const merge, double * const height, int * const order, cluster_result & Z2, const int N) {
  ...
  for (t_index i=0; i<N-1; ++i) {
    ...
    if(node1<N){
      ...
      node_size[i] = node_size[node1-N] + node_size[node2-N];
    }
...
int hclust_fast(int n, double* distmat, int method, int* merge, double* height) {
  ...
  // std::cout << "arrive here end line <----\n";
  
  return 0;
}


double diffFun(const double val1, const double val2){
...
NumericVector ultra_hclust(NumericVector x0, int n_clusts){
  ...
  // std::vector<double> x0 = {1,2,3,4,5,6,7,8,7,6};
  int npoints = x0.size();
  int i, j, k;
  ...
  for (i=0; i<npoints; i++) {
    for (j=i+1; j<npoints; j++) {
      distmat[k] = diffFun(x0[i], x0[j]);
      // std::cout << "distmat[k] --> " << distmat[k] << std::endl;
      k++;
    }
  }
  
  // std::cout << k << " <- size || note: it is here ---> " << *distmat << "\n";
  // clustering call
  int* merge = new int[2*(npoints-1)];
  double* height = new double[npoints-1];
  hclust_fast(npoints, distmat, 4, merge, height);
  
  int* labels = new int[npoints];
  cutree_k(npoints, merge, n_clusts, labels);
...
NumericVector auto_hclust(NumericVector x0){
  ...
  NumericVector heights(npoints-1);
  for(i=0; i<(npoints-1); i++){
    heights[i] = height[i];
  }
  NumericVector tmp = {15, floor(max(heights)/4)};
  double h_cutree = max(tmp);
  int nc_sub = heights.size()+1;
  for(i=0; i < heights.size(); i++){
    if(heights[i] > h_cutree){
      nc_sub = i+1;
      break;
    }
  }
  
  n_clusts = npoints + 1 - nc_sub;
  int* labels = new int[npoints];
  cutree_k(npoints, merge, n_clusts, labels);
...
NumericVector auto_hclust_median(NumericVector x0){
  ...
  int* merge = new int[2*(npoints-1)];
  double* height = new double[npoints-1];
  hclust_fast(npoints, distmat, 3, merge, height);
...
NumericVector matrix_hclust(NumericMatrix data_mtx){
  NumericVector res;
  //std::cout << " --- matrix_hclust --- 0 -- \n";
  int npoints2 = data_mtx.ncol();
  int i, j, k;
  int n_clusts;
  double* distmat2 = new double[(npoints2*(npoints2-1))/2];
  k = 0;
  //std::cout << " --- matrix_hclust --- 1 -- \n";
  for (i=0; i<npoints2; i++) {
    for (j=i+1; j<npoints2; j++) {
      distmat2[k] = data_mtx(i,j);
      k++;
    }
  }
  int* merge2 = new int[2*(npoints2-1)];
  double* height2 = new double[npoints2-1];
  hclust_fast(npoints2, distmat2, 3, merge2, height2);
  // std::cout << " --- matrix_hclust --- 2 -- \n";
  NumericVector heights(npoints2-1);
  for(i=0; i<(npoints2-1); i++){
    heights[i] = height2[i];
  }
  // std::cout << " --- matrix_hclust --- 3 -- \n";
  double h_cutree = 0.25;
  int nc_sub = heights.size()+1;
  for(i=0; i < heights.size(); i++){
    if(heights[i] > h_cutree){
      nc_sub = i+1;
  ...
  n_clusts = npoints2 + 1 - nc_sub;
  int* labels = new int[npoints2];
  cutree_k(npoints2, merge2, n_clusts, labels);
...
// 
// NumericVector matrix_hclust2(NumericVector data_vec, int ncol_num){
//   NumericVector res;
//   std::cout << " --- matrix_hclust --- 0 -- \n";
//   int npoints2 = ncol_num;
//   int i;
//   int n_clusts;
//   double* distmat2 = new double[(npoints2*(npoints2-1))/2];
//   
...
//       distmat2[i] = data_vec[i];
//   }
//   std::cout << " --- matrix_hclust --- 1.5 -- \n";
//   int* merge2 = new int[2*(npoints2-1)];
//   double* height2 = new double[npoints2-1];
//   hclust_fast(npoints2, distmat2, 4, merge2, height2);
//   // std::cout << " --- matrix_hclust --- 2 -- \n";
//   NumericVector heights(npoints2-1);
//   for(i=0; i<(npoints2-1); i++){
//     heights[i] = height2[i];
...
void cutree_k(int n, const int* merge, int nclust, int* labels) {
  ...
  // std::cout << "running into here nicely <---- \n";
  // assign cluster labels
  int label = 0;
  std::vector<int> z(n,-1);
  for (j=0; j<n; j++) {
    if (last_merge[j] == 0) { // still singleton
      // std::cout << "running into here nicely xxx 1 <---- \n";
      labels[j] = label++;
    } else {
      // std::cout << "running into here nicely xxxx 2<---- \n";
      if (z[last_merge[j]] < 0) {
        // std::cout << "running into here nicely xxxx 3<---- \n";
