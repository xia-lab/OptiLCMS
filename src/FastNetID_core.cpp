...
using namespace std;
// A simple struct to keep mz, id, and rt synchronized during sorting
struct Peak {
  std::string id;
  double mz;
  double rt;
...
// [[Rcpp::export]]
DataFrame match_edge_cpp(double temp_fg,
                         NumericVector temp_mz_list,
                         double mz_tol_ppm,
                         double mz_tol_abs,
                         NumericVector temp_RT_list,
                         double temp_deltaRT) {
  
  // Ensure the R vectors have names for ID matching
  if (!temp_mz_list.hasAttribute("names")) {
    stop("temp_mz_list must be a named numeric vector.");
  }
  if (!temp_RT_list.hasAttribute("names")) {
    stop("temp_RT_list must be a named numeric vector.");
  }
  
  CharacterVector mz_names = temp_mz_list.names();
  CharacterVector rt_names = temp_RT_list.names();
  
  // 1. Build an O(1) lookup map for Retention Times (RT)
  std::unordered_map<std::string, double> rt_map;
  int n_rt = temp_RT_list.size();
  for (int k = 0; k < n_rt; ++k) {
    rt_map[as<std::string>(rt_names[k])] = temp_RT_list[k];
  }
  
  // 2. Build and populate Peak objects
  int merge_nrow = temp_mz_list.size();
  std::vector<Peak> peaks(merge_nrow);
  for (int k = 0; k < merge_nrow; ++k) {
    std::string id = as<std::string>(mz_names[k]);
    peaks[k].id = id;
    peaks[k].mz = temp_mz_list[k];
    
    // Lookup RT, if missing assign NA
    if (rt_map.find(id) != rt_map.end()) {
      peaks[k].rt = rt_map[id];
    } else {
      peaks[k].rt = NA_REAL; 
    }
  }
  
  // 3. Sort peaks based on mz and temp_fg
  if (temp_fg > 0) {
  ...
  // Set up C++ vectors to build our final edge list columns
  std::vector<std::string> node1_vec;
  std::vector<std::string> node2_vec;
  std::vector<double> mass_dif_vec;
  
  // 4. Matrix combination logic (using C++ 0-based indexing)
  int i = 0;
  int j = 0;
  int j_pos = 0; // Starts at 0, perfectly aligns with R's 1-based indexing logic
  ...
  while (i < merge_nrow) {
    while (true) {
      ...
      double temp_ms = peaks[j].mz - peaks[i].mz;
      double mass_tol = std::max(std::max(peaks[i].mz, peaks[j].mz) * mz_tol_ppm, mz_tol_abs);
      
      // Fast-forward bounds check
      if (temp_fg > 0) {
        if (temp_ms < (temp_fg - mass_tol)) {
          j_pos = j; // locate the last j that has smaller ms
          continue;  // identical to 'next' in R
        }
      } else {
        if (temp_ms > (temp_fg + mass_tol)) {
          j_pos = j;
      ...
      // Criteria to entry
      if (std::abs(temp_ms - temp_fg) < mass_tol) {
        double delta_RT = peaks[j].rt - peaks[i].rt;
        if (std::abs(delta_RT) < temp_deltaRT) {
          node1_vec.push_back(peaks[i].id);
          node2_vec.push_back(peaks[j].id);
          mass_dif_vec.push_back((temp_ms - temp_fg) / peaks[j].mz * 1e6);
        }
      }
      
      // Upper limit bounds check / Break
      if (temp_fg > 0) {
  ...
  // 5. Return as a DataFrame for maximum R efficiency
  return DataFrame::create(
    Named("node1") = node1_vec,
    Named("node2") = node2_vec,
    Named("mass_dif") = mass_dif_vec,
