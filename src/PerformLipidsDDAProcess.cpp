...
List PerformLipidDDA_main(NumericMatrix pm, 
                    ...
                    string file_nm) {
  if(showOutput){
    cout << "Running into PerformLipidDDA_main <----==== " << endl;
  }
  lipids_ddaprocess runObj;
  runObj.lipidsSetDDA_Arguments(pm, scant1, scant2, scanms1, scanms2, prec_mzs, 
                          win_size, ppm1, ppm2, sn, filt, showOutput, thread_num, file_nm, useEntropy,
                          0, intensity_thresh, ionmode, db_path);
  runObj.lipidsPerformDDAProcess_core(decoOn, showOutput);
